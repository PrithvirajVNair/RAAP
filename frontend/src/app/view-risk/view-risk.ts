...
export class ViewRisk {
  ...
  constructor(private route: ActivatedRoute, private api: Api, private fb: FormBuilder) {
  ...
  editRisk() {
    if (this.riskForm.valid) {
      ...
      this.api.updateRiskAPI({ _id: this.risk()._id, title, description, category, impact, likelihood, solution, mitigationStatus, dueDate }).subscribe({
        next: (res: any) => {
          console.log(res);
          alert(res)
          location.reload()
